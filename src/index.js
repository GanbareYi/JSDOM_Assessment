...
function renderContact(contact) {
  const {id, name, email, picture, address:{street, suite, city, zipcode, geo: {lat, lng}},
          phone, website, company: {companyName, catchPhrase, bs}} = contact;
  const content = 
...
*/
function render(contactsArr) {

  const contactSection = document.querySelector("#contacts");

  contactSection.innerHTML = "";

  const content = contactsArr.map(contact => renderContact(contact)).join("");
...
*/
function loadCities(contactsArr) {
  const selectElement = document.querySelector("#filterOptions");

  selectElement.innerHTML = '<option value="0">-- Select a city --</option>';

  if (contactsArr.length !== 0) {
    const citys = contactsArr.map(contact => contact.address.city);
    const cityOptns = citys.filter((city, index) => citys.indexOf(city) === index);

    cityOptns.forEach(city => {
